...
func TestAuthorityType(t *testing.T) {
	...
	auth.Status.StoreUint8( constants.IDENTITY_FEDERATED_SERVER)
	if auth.Type() != 1 {
		t.Errorf("Invalid type returned - %v", auth.Type())
	}

	auth.Status.StoreUint8( constants.IDENTITY_AUDIT_SERVER)
	if auth.Type() != 0 {
		t.Errorf("Invalid type returned - %v", auth.Type())
	}
...
func TestAuthoritySignature(t *testing.T) {
	...
	s.Authorities[index].Status.StoreUint8( 1)
...
func TestSameAuth(t *testing.T) {
	...
	a.Status.StoreUint8( b.status.LoadUint8() + 1)
	if a.IsSameAs(b) {
		t.Error("Diff Status chains, should be different")
	}
	a.Status.StoreUint8( b.Status.LoadUint8())
