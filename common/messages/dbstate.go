...
func (m *DBStateMsg) Validate(state interfaces.IState) int {
	if !m.DBHash.IsSameAs(m.DirectoryBlock.GetHash()) 	{ return -1 }
	if !m.ABHash.IsSameAs(m.AdminBlock.GetHash()) 		{ return -1 }
	if !m.FBHash.IsSameAs(m.FactoidBlock.GetHash()) 	{ return -1 }
	if !m.ECHash.IsSameAs(m.EntryCreditBlock.GetHash()) { return -1 }
	return 1
}
