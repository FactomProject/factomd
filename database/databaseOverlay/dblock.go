...
func (db *Overlay) ProcessDBlockBatch(dblock DatabaseBatchable) error {
...
func (db *Overlay) FetchBlockHeightByKeyMR(sha IHash, dst DatabaseBatchable) (int64, error) {
...
func (db *Overlay) FetchDBlockByKeyMR(keyMR IHash, dst DatabaseBatchable) (DatabaseBatchable, error) {
	return db.FetchBlock([]byte{byte(TBL_DB)}, keyMR, dst)
}

// FetchDBlockByHeight gets an directory block by height from the database.
func (db *Overlay) FetchDBlockByHeight(dBlockHeight uint32, dst DatabaseBatchable) (DatabaseBatchable, error) {
...
func (db *Overlay) FetchDBlockByHash(dBMR IHash, dst DatabaseBatchable) (DatabaseBatchable, error) {
	return db.FetchBlockBySecondaryIndex([]byte{byte(TBL_DB_MR)}, []byte{byte(TBL_DB)}, dBMR, dst)
}

// FetchAllDBlocks gets all of the fbInfo
func (db *Overlay) FetchAllDBlocks(sample BinaryMarshallableAndCopyable) ([]BinaryMarshallableAndCopyable, error) {
