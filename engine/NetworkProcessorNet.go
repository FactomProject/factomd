				...
				if err == nil && msg != nil {
					...
					if fnode.State.Replay.IsTSValid_(msg.GetMsgHash().Fixed(),
						...
						if len(fnode.State.InMsgQueue()) < 9000 {
							if msg.Type() == 18 {
								fmt.Println(fnode.State.FactomNodeName, "!!!!!!!!!!a")
							}
							fnode.State.InMsgQueue() <- msg
						}

					} else {
						fmt.Println(fnode.State.FactomNodeName, "???", msg.Type())
...
func NetworkOutputs(fnode *FactomNode) {
	for {
		...
		fmt.Println(fnode.State.FactomNodeName, "MSG")
		if fnode.State.FactomNodeName == "FNode3" { //} && fnode.State. { //} && len(fnode.Peers) > 4 {
			h, err := primitives.HexToHash("9433b714f22cb3994a89340eb1a3b7cdb36c5e7cab8c5c099a7420393c7c3a33")
			if err == nil {
