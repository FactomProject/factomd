...
func TestDBStateMapping(t *testing.T) {
	...
	inputEvent := events.EventFromNetworkMessage(msg, eventmessages.EventSource_COMMIT_DIRECTORY_BLOCK)
...
func TestCommitChainMapping(t *testing.T) {
	...
	inputEvent := events.EventFromNetworkMessage(msg, eventmessages.EventSource_ADD_TO_PROCESSLIST)
	event, err := eventservices.MapToFactomEvent(inputEvent)
	if err != nil {
		t.Error(err)
	}
...
func TestCommitEntryMapping(t *testing.T) {
	...
	inputEvent := events.EventFromNetworkMessage(msg, eventmessages.EventSource_ADD_TO_HOLDING)
	event, err := eventservices.MapToFactomEvent(inputEvent)
	if err != nil {
		t.Error(err)
	}
