...
func NewEventService(state events.StateEventServices, config *util.FactomdConfig, factomParams *globals.FactomParams) events.EventServiceControl {
...
func (esi *eventServiceInstance) Send(event eventinput.EventInput) error {
	if esi.parentState.GetRunState() > runstate.Running { // Stop queuing messages to the events channel when shutting down
		return nil
	}

	// Only send info messages when EventReplayDuringStartup is disabled
	if !esi.params.ReplayDuringStartup && !esi.parentState.IsRunLeader() {
		switch event.(type) {
	...
	factomEvent.IdentityChainID = esi.parentState.GetIdentityChainID().Bytes()
	select {
