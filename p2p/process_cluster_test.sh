...
if [ $? -eq 0 ]; then
    ...
    echo "Running..."
    factomd -exclusive=true -network="TEST" -networkPort=8118 -peers="127.0.0.1:8119" -netdebug=2 > testing/node1.out & node0=$!
    sleep 6 
    factomd -exclusive=true -network="TEST" -prefix="test2-" -port=9121 -networkPort=8119 -peers="127.0.0.1:8118" -netdebug=2  > testing/node2.out  & node1=$!
    # sleep 6
