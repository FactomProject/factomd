...
func (vm *VM) ReportMissing(height int, delay int64) {
	...
	if delay < vm.p.State.FactomSecond()/time.Millisecond {
		delay = vm.p.State.FactomSecond() / time.Millisecond // Floor for delays is 1 second so there is time to merge adjacent requests
	}
