...
//returns status, transaction timestamp, block timestamp, and an error
func (s *State) GetACKStatus(hash interfaces.IHash) (int, interfaces.Timestamp, interfaces.Timestamp, error) {
	m, found := s.ProcessLists.LastList().OldMsgs[hash.Fixed()]
	if found {
		return constants.AckStatusACK, m.GetTimestamp(), nil, nil
	}

	for _, tx := range s.ProcessLists.LastList().NewEntries {
		if hash.IsSameAs(tx.GetHash()) {
			return constants.AckStatusACK, nil, s.ProcessLists.LastList().DirectoryBlock.GetHeader().GetTimestamp(), nil
		}
	}
	...
	if ecBlock != nil {
		...
		if tx != nil {
			return constants.AckStatusACK, tx.GetTimestamp(), s.ProcessLists.LastList().DirectoryBlock.GetHeader().GetTimestamp(), nil
		}
	}
	...
	if msg != nil {
		return constants.AckStatusInvalid, nil, nil, nil
	}
	...
	if in == nil {
		if s.IsStateFullySynced() {
			return constants.AckStatusNotConfirmed, nil, nil, nil
		} else {
			return constants.AckStatusUnknown, nil, nil, nil
		}
	}
	...
	if fBlock != nil {
		tx := fBlock.GetTransactionByHash(hash)
		if tx == nil {
			return 0, nil, nil, fmt.Errorf("Transaction not found in a block we were expecting")
		}
		return constants.AckStatusDBlockConfirmed, tx.GetTimestamp(), dBlock.GetHeader().GetTimestamp(), nil
	}
	...
	return constants.AckStatusDBlockConfirmed, nil, dBlock.GetHeader().GetTimestamp(), nil
}
