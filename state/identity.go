...
func (st *State) GetSigningKey(id interfaces.IHash) (interfaces.IHash, int) {
	...
	for _, identity := range st.Identities {
		if identity.IdentityChainID.IsSameAs(id) {
			return identity.SigningKey, getReturnStatInt(identity.Status.LoadUint8())
		}
	}
	return nil, -1
}
...
func (st *State) removeIdentity(i int) {
	if st.Identities[i].Status.LoadUint8() == constants.IDENTITY_SKELETON {
...
func (st *State) CreateBlankFactomIdentity(chainID interfaces.IHash) int {
	...
	oneID.Status.StoreUint8( constants.IDENTITY_UNASSIGNED)
	if chainID.IsSameAs(st.GetNetworkSkeletonIdentity()) {
		oneID.Status.StoreUint8( constants.IDENTITY_SKELETON)
	}
...
func checkIdentityForFull(identityIndex int, st *State) error {
	status := st.Identities[identityIndex].Status.LoadUint8()
...
func ProcessIdentityToAdminBlock(st *State, chainID interfaces.IHash, servertype int) bool {
	...
	if index != -1 {
		...
		if id.SigningKey == nil || id.SigningKey.IsSameAs(zero) {
			...
			if !statusIsFedOrAudit(id.Status.LoadUint8()) {
				st.removeIdentity(index)
			}
			return true
		} else {
		...
		if servertype == 0 {
			id.Status.StoreUint8( constants.IDENTITY_PENDING_FEDERATED_SERVER)
		} else if servertype == 1 {
			id.Status.StoreUint8( constants.IDENTITY_PENDING_AUDIT_SERVER)
		}
	...
	if servertype == 0 {
		...
		st.Identities[index].Status.StoreUint8( constants.IDENTITY_PENDING_FEDERATED_SERVER)
	} else if servertype == 1 {
		st.LeaderPL.AdminBlock.AddAuditServer(chainID)
		st.Identities[index].Status.StoreUint8( constants.IDENTITY_PENDING_AUDIT_SERVER)
	}
...
func UpdateIdentityStatus(ChainID interfaces.IHash, StatusTo uint8, st *State) {
	...
	st.Identities[IdentityIndex].Status.StoreUint8( StatusTo)
}
